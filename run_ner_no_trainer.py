...
def main():
    ...
    def get_label_list(labels):
        ...
        if args.label_schema == "IO":
            with open(args.label_list, "r") as f:
                label_list = [l.strip() for l in f.readlines() if not l.strip().startswith("B-")]
        else:
            with open(args.label_list, "r") as f:
    ...
    if args.label_schema == "IO":
        import copy
        ori_label_to_id = copy.deepcopy(label_to_id)
        ori_label_list = copy.deepcopy(label_list)
        for label, id in label_to_id.items():
            if label != "O" and "B-"+label[2:] not in label_to_id:
                ori_label_to_id["B-"+label[2:]] = len(ori_label_to_id)
                ori_label_list.append("B-"+label[2:])
    else:
        ori_label_list = label_list
        ori_label_to_id = label_to_id
        ...
        for i, label in enumerate(examples[label_column_name]):
            ...
            for word_idx in word_ids:
                ...
                # We set the label for the first token of each word.
                    ...
                    ori_label_ids.append(ori_label_to_id[label[word_idx]])
                ...
                # the label_all_tokens flag.
                    ...
                    ori_label_ids.append(ori_label_to_id[label[word_idx]] if args.label_all_tokens else -100)
                    if args.label_schema == "IO":
    ...
    retain_key = "" #classifier
    optimizer_grouped_parameters = [
        {
            "params": [p for n, p in model.named_parameters() if not any(nd in n for nd in no_decay) and retain_key in n],
            "weight_decay": args.weight_decay,
        },
        {
            "params": [p for n, p in model.named_parameters() if any(nd in n for nd in no_decay) and retain_key in n],
            "weight_decay": 0.0,
        },
    ]
    ...
    def get_labels(predictions, references, tokens):
        ...
        true_predictions = [
            [ori_label_list[p] for (p, l) in zip(pred, gold_label) if l != -100]
            for pred, gold_label in zip(y_pred, y_true)
        ]

        true_labels = [
            [ori_label_list[l] for (p, l) in zip(pred, gold_label) if l != -100]
            for pred, gold_label in zip(y_pred, y_true)
        ]

        if args.label_schema == "IO":
            true_predictions = list(map(switch_to_BIO, true_predictions))
            true_labels = list(map(switch_to_BIO, true_labels))
